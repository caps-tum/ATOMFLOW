...
COMP_COL_END           = 0xD4
MOVE_COUNT_REG         = 0xDC
# All offsets below confirmed against xatomflow_controller_hw.h regenerated by
# Vitis HLS 2024.2 csynth (2026-08-31):
#   status 0xEC, targetsFilled 0xFC, targetsRequired 0x10C.
# Adding these ports left every pre-existing offset (0x10..0xDC) unchanged.
# Do NOT predict these values — re-read the generated header after any change
# to the top-level signature.
STATUS_REG             = 0xEC
TARGETS_FILLED_REG     = 0xFC
TARGETS_REQUIRED_REG   = 0x10C

# ATOMFLOW_STATUS_* codes — must match atomflow_controller.hpp.
# A read of 0 means the register was never written: stale bitstream
# without the status port, or the kernel has not completed.
ATOMFLOW_STATUS = {
    0: "NOT-WRITTEN (stale bitstream without status port?)",
    1: "OK",
...
# controller rejects anything else (ATOMFLOW_STATUS_ERR_IMAGE_CFG).
# Must match PSF_WINDOW / PSF_SUPERSAMPLE_ONLY / PIXEL in image_analysis.hpp.
PSF_WINDOW           = 31
PSF_SUPERSAMPLE_ONLY = 1
IMAGE_STRIDE         = 1024   # required row stride of the fullImage buffer
STATUS_OK = 1
...
# ParallelMove wire layout — MUST match sortLatticeByRow.hpp.
#   MAX_STEPS  <- MAX_MOVE_STEPS      (sortLatticeByRow.hpp)
#   MAX_SEL    <- AOD_COL_LIMIT / AOD_ROW_LIMIT
# Step layout: int16 colSelection[16] | int16 rowSelection[16] | u8 cols | u8 rows
#            = 32 + 32 + 1 + 1 = 66 bytes
# ParallelMove: MAX_STEPS*66 + 1 (stepsCount), padded to int16 alignment
# Wire size:    ceil(sizeof(ParallelMove)/64)*64  -> this is the AXI-Stream TLAST
#               value that create_bd.tcl must configure (DEFAULT_TLAST).
MAX_STEPS          = 4
MAX_SEL            = 16
STEP_SIZE          = 66
STEPS_COUNT_OFF    = MAX_STEPS * STEP_SIZE            # 264: stepsCount byte
FLAGS_OFF          = STEPS_COUNT_OFF + 1              # 265: per-move flags byte
PARALLEL_MOVE_SIZE = FLAGS_OFF + 1                    # 266

# ParallelMove flag bits — must match MOVE_FLAG_* in sortLatticeByRow.hpp.
# DISCARD: at least one tone of this move ends outside the trap array, so the
# atoms it carries leave the system. This is stated explicitly rather than
# being inferred from out-of-range destination coordinates.
MOVE_FLAG_DISCARD = 0x01
BEATS_PER_MOVE     = (PARALLEL_MOVE_SIZE + 63) // 64  # 5
BYTES_PER_MOVE     = BEATS_PER_MOVE * 64              # 320
...
def decode_move(raw):
    ...
    if len(raw) < STEPS_COUNT_OFF + 1:
        return None
    steps_count = raw[STEPS_COUNT_OFF]
    if steps_count == 0 or steps_count > MAX_STEPS:
        return None
    flags = raw[FLAGS_OFF] if len(raw) > FLAGS_OFF else 0
    steps = []
...
def main():
    ...
    assert atom_count == ZONE_R * ZONE_C, (
        f"Expected {ZONE_R}x{ZONE_C} zone detections, got {atom_count}")

    # Fail here rather than letting the controller reject the run: these are the
    # only image parameters the reconstruction datapath implements, and catching
    # them host-side gives a far clearer message than ERR_IMAGE_CFG.
    assert proj_shape0 == PSF_WINDOW and proj_shape1 == PSF_WINDOW, (
        f"IP implements a fixed {PSF_WINDOW}x{PSF_WINDOW} PSF window, "
        f"got ({proj_shape0},{proj_shape1})")
    assert psf_super == PSF_SUPERSAMPLE_ONLY, (
        f"IP implements psfSupersample={PSF_SUPERSAMPLE_ONLY} only, got {psf_super}")
    assert 0 < full_rows <= IMAGE_STRIDE and 0 < full_cols <= IMAGE_STRIDE, (
        f"image {full_rows}x{full_cols} exceeds the {IMAGE_STRIDE}x{IMAGE_STRIDE} buffer")

    # Every PSF window must sit fully inside the image; the controller refuses
    # the run otherwise (ERR_ATOM_OOB) because a negative window origin becomes
    # a wild AXI address in the extraction path.
    half = PSF_WINDOW // 2
    _bad = [(i, ax, ay)
            for i, (ax, ay) in enumerate(input_data["atomLocations"][:atom_count])
            if not (half <= round(ax) < full_cols - half and
                    half <= round(ay) < full_rows - half)]
    assert not _bad, (
        f"{len(_bad)} atom(s) sit within {half}px of the image edge, "
        f"e.g. site {_bad[0][0]} at ({_bad[0][1]}, {_bad[0][2]})")
    ...
    print(f"\nTarget: {np.sum(target_flat)} sites in {ZONE_R}x{ZONE_C} zone")
    print(f"Physical grid: {GRID_R}x{GRID_C}, comp zone: rows [{COMP_R0},{COMP_R1}) x cols [{COMP_C0},{COMP_C1})")
    ...
    print(f"\u2713 {elapsed_ms:.1f} ms | moveCount={move_count} | FIFO packets={len(all_raw_moves)}"
          f" | status={status} ({ATOMFLOW_STATUS.get(status, 'UNKNOWN')})"
          f" | targets={t_filled}/{t_required}")
    if status != STATUS_OK:
        print("WARNING: controller status is not OK — moveCount/stream must not be trusted")
    if t_required and t_filled < t_required:
        print(f"WARNING: target geometry only {t_filled}/{t_required} filled")
    ...
    for run in range(N_BENCH_RUNS):
        ...
        st = ctrl.read(STATUS_REG)
        tf = ctrl.read(TARGETS_FILLED_REG)
        tr = ctrl.read(TARGETS_REQUIRED_REG)
        if st != STATUS_OK:
            print(f"Run {run}: WARNING status={st} ({ATOMFLOW_STATUS.get(st, 'UNKNOWN')})"
                  f" targets={tf}/{tr}")
